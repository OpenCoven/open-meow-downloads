...
**OpenMeow** puts ambient AI in the one place every pro looks at all day — the notch of your MacBook Pro. Hit `⌘-space` from anywhere and a native shell drops out of the notch: ask a question and the answer streams inline, or hand a task to a browser agent and watch it drive real Chromium against the live web. No tab switch. No sidebar. No Electron.
...
3. **Launch** from Applications. Grant permission prompts on first run:
   - **Accessibility** — required for the `⌘-space` global shortcut to work system-wide.
...
| **Expanded** | `⌘-space` | Grows vertically up to 7 lines | Quick prompts, agent chips |
...
- **First paint under 200ms** from `⌘-space` — streaming tokens render as they arrive, nothing blocks the UI thread.
